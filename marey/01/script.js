...
// d3.loadData('2017-06-15.tsv', function(err, res){
  ...
  c = d3.conventions({parentSel: d3.select('#graph').html(''), margin: {left: 50}, width: 4000})
  stations = _.sortBy(_.uniq(data456.map(d => d.station)))

  c.x.domain(d3.extent(data456, d => d.arrivalTime))
  // c.x.domain([1497501570000, 1497591180000])
  c.y.domain([stations.length, 0])

  station2y = {}
  stations.forEach((d, i) => station2y[d] = c.y(i))

  c.xAxis.tickFormat(d3.timeFormat('%H:%M'))
  c.yAxis.tickFormat(d => stations[d])

  c.drawAxis()

  
  var rScale = d3.scaleSqrt().domain([0, 1000*60*10]).range([1, 2]).clamp(true)

  var line = d3.line()
    .x(d => c.x(d.arrivalTime))
    .y(d => station2y[d.station])
  ...
  byTrainSel
    .filter((d, i) => i < 500)
    .appendMany(d => d.filter(d => d.absDif > 1000*60), 'circle')
    .translate(d => [c.x(d.arrivalTime), station2y[d.station]])
...
d3.loadData('transit-appx.json', function(err, res){
  return
  var data = res[0]

  var parseTime = d3.timeParse('%Y-%m-%d %H:%M:%S')
  data.forEach(function(d){
    d.arrivalTime = parseTime(d.arrivalTime)
    d.station = +d.stopId.replace('N', '').replace('S', '')
    d.route = d.routeShortName

    d.day = d3.timeFormat('%d')(d.arrivalTime)
    d.tstamp = d3.timeFormat('%d %H:%M')(d.arrivalTime)
  })

  data456 = data.filter(stopFilter)
  data456 = _.sortBy(data456, d => d.arrivalTime)

  byTrain = d3.nestBy(data456, d => d.rtTripId)
  byTrain = _.sortBy(byTrain, d => d[0].arrivalTime)
    .filter(d => d.every(dayFilter))
    .filter(byTrainFilter)
  ...
  c = d3.conventions({parentSel: d3.select('#graph2').html(''), margin: {left: 50}, width: 8000})
  stations = _.sortBy(_.uniq(data456.map(d => d.station)))
